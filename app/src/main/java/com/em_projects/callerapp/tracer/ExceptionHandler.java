package com.em_projects.callerapp.tracer;
...
public class ExceptionHandler {
    ...
    public static boolean register(Context context) {
        ...
        try {
            ...
            //Brand brand = BrandingManager.getBranding(pi.packageName, context);

            boolean isDebuggable = (0 != (context.getApplicationInfo().flags &= ApplicationInfo.FLAG_DEBUGGABLE));

            G.STATE = (isDebuggable) ? "DEBUG (TESTING)" : "RELEASE";
            // App name
            G.APP_NAME = "ListenApp";
...
//	}
    private static String[] searchForStackTraces() {
        ...
        for (String filename : filenames) {
            if (filename.endsWith(".stacktrace")) {
                result.add(filename);
            }
        }
        ...
        try {
            ...
            if (list != null && list.length > 0) {
                ...
                for (int i = 0; i < list.length; i++) {
                    ...
                    // "packagename-version-...."
                    final String version = list[i].split("-")[0];
                    ...
                    // Dangerous assuming application context was initialized
                    /// instantiation
//					HttpSingleton.getInstance().getRequestQueue();
//					RequestQueue queue = HttpSingleton.getInstance().getRequestQueue();
//
//
//                    final String av = androidVersion ;
//                    final String pm = phoneModel ;
//
//                    RequestFuture<String> future = RequestFuture.newFuture();
////        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, urlString, "", future, future)
////					StringRequest stringRequest = new StringRequest(Request.Method.POST, G.URL,future, future);
//
//					StringRequest sr = new StringRequest(Request.Method.POST,G.URL,
...
//							params.put("package_name",G.APP_PACKAGE + "~" + G.APP_NAME + "~" + G.STATE);
//							params.put("package_version", version);
//							params.put("phone_model", pm);
//							params.put("android_version",av);
...
//					queue.add(sr);
                    ...
                    nvps.add(new BasicNameValuePair("package_name",
                            G.APP_PACKAGE + "~" + G.APP_NAME + "~" + G.STATE));
